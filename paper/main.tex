...
% ------------------------------------------------------------
\renewcommand{\papertitle}{WARP Graphs: Rulial Distance \& Observer Geometry}
\renewcommand{\papernumber}{Paper IV}
...
\AIONFrontMatter{This paper develops a geometry of \emph{observers} on deterministic WARP computation,
concluding the mathematical core of the \AION{} Foundations Series.
Building on deterministic replay (unique re-execution of the committed tick sequence from boundary data;
Paper~II~\cite{Ros25b}) and holographic provenance payloads (Paper~III~\cite{Ros25c}), we model an observer as a
resource-bounded functor from a history category
$\Hist(\mathcal{U},R)$ into an observation space (symbol streams, trace graphs, certificates, \emph{etc}.).
Different observers legitimately induce different causal narratives of the same underlying \WARP\ worldline:
they may coarse-grain, forget, compress, or re-express provenance.
We introduce a translation framework between observers, measure translator complexity using
Minimum Description Length (MDL)~\cite{Ris78}, and define a budgeted, MDL-regularised distance
between observers: the \emph{rulial distance} $D_{\tau,m}$.
Under standard subadditivity and non-expansiveness hypotheses, the underlying directed translation cost admits a
Lawvere-metric (enriched-category) interpretation that makes composition coincide with the triangle inequality,
and the symmetrised $D_{\tau,m}$ is a symmetric quasi-pseudometric (i.e.\ it satisfies the triangle inequality
up to a constant additive offset due to prefix-coding overhead)
that is monotone under budget relaxation.
We relate deterministic \WARP\ rewriting to multiway systems and the Ruliad in the sense of Wolfram~\cite{Wolfram2020},
and formalise the Chronos--Kairos--Aion triad as a three-layer time model
(linear time, branch events, and the encompassing possibility space).
We align this triad with a minimal temporal logic over histories, illustrating how ``always'', ``eventually'', and ``until''
express liveness and reconciliation properties of \WARP\ computations and how such claims transport across observers under
low-distortion translation.
The resulting observer geometry provides a computable notion of ``frame separation''---the cost
of translating between descriptions of the same computation.
}
...
The first three papers in the \AION{} Foundations Series establish a deterministic, provenance-carrying
graph rewriting semantics on recursively nested state.
...
\begin{figure}[t]
  \centering
	  \begin{tikzpicture}[
	      ...
	      maybe/.style={-Latex,thick,draw=black!70,dash pattern=on 4pt off 2pt},
	      >=Latex
	    ]
...
\emph{Aion time} is the encompassing possibility space: the full multiway history category $\Hist(\mathcal{U},R)$,
or, at the largest scale, the Ruliad history space $\Ruliad$.
\end{definition}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Ethics (Paper V).} Once perfect provenance and deterministic replay are available, the principal questions are social:
        accountability, privacy, governance, and the power implied by reversible history.
